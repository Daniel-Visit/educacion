...
async function verificarIntegridadDB() {
  try {
    ...
    console.log(`   ✅ Relaciones profesor-asignatura: ${profesorAsignaturas.length} registros`);
    ...
    console.log(`   ✅ Relaciones profesor-nivel: ${profesorNiveles.length} registros`);
    ...
    console.log(`   ✅ Planificaciones anuales: ${planificaciones.length} registros`);
    ...
    console.log(`   ✅ OAs con asignatura válida: ${oasConRelaciones.length - oasSinAsignatura.length}/${oasConRelaciones.length}`);
    console.log(`   ✅ OAs con nivel válido: ${oasConRelaciones.length - oasSinNivel.length}/${oasConRelaciones.length}`);
    ...
    const evaluacionesSinMatriz = evaluacionesConRelaciones.filter(eval => !eval.matriz);
    const evaluacionesSinArchivo = evaluacionesConRelaciones.filter(eval => !eval.archivo);
    console.log(`   ✅ Evaluaciones con matriz válida: ${evaluacionesConRelaciones.length - evaluacionesSinMatriz.length}/${evaluacionesConRelaciones.length}`);
    console.log(`   ✅ Evaluaciones con archivo válido: ${evaluacionesConRelaciones.length - evaluacionesSinArchivo.length}/${evaluacionesConRelaciones.length}`);
    ...
    const horariosSinProfesor = horariosConRelaciones.filter(hor => !hor.profesor);
    const horariosSinAsignatura = horariosConRelaciones.filter(hor => !hor.asignatura);
    const horariosSinNivel = horariosConRelaciones.filter(hor => !hor.nivel);
    console.log(`   ✅ Horarios con profesor válido: ${horariosConRelaciones.length - horariosSinProfesor.length}/${horariosConRelaciones.length}`);
    console.log(`   ✅ Horarios con asignatura válida: ${horariosConRelaciones.length - horariosSinAsignatura.length}/${horariosConRelaciones.length}`);
    console.log(`   ✅ Horarios con nivel válido: ${horariosConRelaciones.length - horariosSinNivel.length}/${horariosConRelaciones.length}`);

    console.log('\n🎉 Verificación de integridad completada exitosamente!');
    console.log('✅ La base de datos está en buen estado y lista para el prisma db push.');
