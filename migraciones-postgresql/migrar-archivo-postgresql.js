...
async function migrarArchivo() {
  try {
    ...
    const existentes =
      await prismaPostgres.$queryRaw`SELECT COUNT(*) as total FROM "Archivo"`;
    ...
    for (const archivo of archivos) {
      ...
      `;
      console.log(
        `✅ Insertado: ${archivo.titulo} (ID: ${archivo.id}, Tipo: ${archivo.tipo})`
      );
    }
    ...
    const ejemplos =
      await prismaPostgres.$queryRaw`SELECT id, titulo, tipo, LENGTH(contenido) as contenido_length FROM "Archivo" ORDER BY id LIMIT 5`;
