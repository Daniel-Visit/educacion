...
async function migrarAsignaturaNivel() {
  try {
    ...
    const existentes = await prismaPostgres.$queryRaw`SELECT COUNT(*) as total FROM asignatura_nivel`;
    ...
    for (const asignaturaNivel of asignaturaNiveles) {
      ...
      `;
      console.log(`✅ Insertado: ${asignaturaNivel.codigo} - ${asignaturaNivel.descripcion}`);
    }

    // 4. Verificar migración
    console.log('\n🔍 Verificando migración...');
    const migrados = await prismaPostgres.$queryRaw`SELECT COUNT(*) as total FROM asignatura_nivel`;
    ...
    console.log('\n📋 Datos migrados:');
    const datosMigrados = await prismaPostgres.$queryRaw`SELECT * FROM asignatura_nivel ORDER BY id`;
