...
async function verificarPlanificacionGuardada() {
  try {
    ...
    const planificacionResponse = await fetch('http://localhost:3000/api/planificaciones/2');
    const planificacion = await planificacionResponse.json();
    
    console.log('Planificación:', planificacion.nombre);
    console.log('Horario - Asignatura:', planificacion.horario.asignatura.nombre);
    ...
    planificacion.asignaciones.forEach((asignacion, index) => {
      console.log(`${index + 1}. OA ID: ${asignacion.oaId} | ${asignacion.oa.oas_id} | ${asignacion.oa.descripcion_oas.substring(0, 50)}... | Clases: ${asignacion.cantidadClases}`);
    });
    
    // 3. Verificar si OA 01 está en las asignaciones
    console.log('\n3. Buscando OA 01 específicamente:');
    const oa01 = planificacion.asignaciones.find(asignacion => asignacion.oa.oas_id === 'OA 01');
    if (oa01) {
