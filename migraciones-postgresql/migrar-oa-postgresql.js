...
async function migrarOA() {
  try {
    ...
    const oasExistentes = await prismaPostgres.$queryRaw`SELECT COUNT(*) as total FROM oa`;
    ...
    for (const oa of oas) {
      ...
      `;
      console.log(`✅ Insertado: ${oa.oas_id} - ${oa.descripcion_oas.substring(0, 50)}... (basal: ${basalBoolean})`);
    }

    // 4. Verificar migración
    console.log('\n🔍 Verificando migración...');
    const oasMigrados = await prismaPostgres.$queryRaw`SELECT COUNT(*) as total FROM oa`;
    ...
    const ejemplos = await prismaPostgres.$queryRaw`SELECT id, oas_id, descripcion_oas, basal FROM oa ORDER BY id LIMIT 3`;
