  ...
  try {
    ...
    ejes.forEach((eje, ejeIndex) => {
      ...
      eje.oas.forEach((oa, oaIndex) => {
        console.log(
          `  ${oaIndex + 1}. ID: ${oa.id} | ${oa.oas_id} | ${oa.descripcion_oas.substring(0, 50)}...`
        );
      });
    });
    ...
    const oasLenguaje = ejes
      .flatMap(eje => eje.oas)
      .filter(oa => oa.asignatura_id === 7);
    oasLenguaje.forEach((oa, index) => {
      console.log(
        `  ${index + 1}. ID: ${oa.id} | ${oa.oas_id} | ${oa.descripcion_oas.substring(0, 50)}...`
      );
    });

    // 4. Verificar OAs de Matemática específicamente
    console.log('\n4. OAs de Matemática (asignatura_id: 9):');
    const oasMatematica = ejes
      .flatMap(eje => eje.oas)
      .filter(oa => oa.asignatura_id === 9);
