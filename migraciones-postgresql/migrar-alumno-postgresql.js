...
async function migrarAlumno() {
  try {
    ...
    const existentes = await prismaPostgres.$queryRaw`SELECT COUNT(*) as total FROM alumno`;
    ...
    for (const alumno of alumnos) {
      ...
      `;
      console.log(`✅ Insertado: ${alumno.nombre} ${alumno.apellido} (RUT: ${alumno.rut})`);
    }

    // 4. Verificar migración
    console.log('\n🔍 Verificando migración...');
    const migrados = await prismaPostgres.$queryRaw`SELECT COUNT(*) as total FROM alumno`;
    ...
    const ejemplos = await prismaPostgres.$queryRaw`SELECT id, rut, nombre, apellido, curso, "nivelId" FROM alumno ORDER BY id LIMIT 5`;
