...
async function migrarPreguntaIndicador() {
  try {
    ...
    const existentes = await prismaPostgres.$queryRaw`SELECT COUNT(*) as total FROM pregunta_indicador`;
    ...
    for (const preguntaIndicador of preguntaIndicadores) {
      ...
      `;
      console.log(`✅ Insertado: ID ${preguntaIndicador.id} - Pregunta: ${preguntaIndicador.preguntaId}, Indicador: ${preguntaIndicador.indicadorId}, Tipo: ${preguntaIndicador.tipo}`);
    }

    // 4. Verificar migración
    console.log('\n🔍 Verificando migración...');
    const migrados = await prismaPostgres.$queryRaw`SELECT COUNT(*) as total FROM pregunta_indicador`;
    ...
    console.log('\n📋 Datos migrados:');
    const datosMigrados = await prismaPostgres.$queryRaw`SELECT * FROM pregunta_indicador ORDER BY id`;
