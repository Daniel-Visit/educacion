...
export default function EstadoEvaluacion({
  estado,
  className = '',
}: EstadoEvaluacionProps) {
  const getIcon = () => {
    switch (estado) {
      case 'completa':
  ...
  return (
    <div
      className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium ${obtenerColorEstado(estado)} ${className}`}
    >
      {getIcon()}
