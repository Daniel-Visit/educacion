...
export const calcularNota = (porcentajeCorrectas: number, nivelExigencia: number): number => {
...
export const calcularEstadisticas = (
  ...
  const notas = respuestasAlumnos.map(r => calcularNota(r.porcentaje, nivelExigencia));
  
  // Calcular promedio de notas
  const promedioNota = Math.round((notas.reduce((sum, nota) => sum + nota, 0) / notas.length) * 100) / 100;
...
export const calcularRangosPorcentajes = (respuestasAlumnos: RespuestaAlumno[]) => {
  const rangos = [
...
export const generarCSV = (
  ...
  rows.push(['Porcentaje Aprobación', `${stats.porcentajeAprobacion}%`, '', '', '', '']);
...
export const descargarCSV = (csvContent: string, nombreEvaluacion: string) => {
  ...
  link.setAttribute('download', `resultados_${nombreEvaluacion.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().split('T')[0]}.csv`);
