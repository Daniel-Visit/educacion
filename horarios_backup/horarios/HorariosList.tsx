      ...
      {horarios.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-10">
          {horarios.map(horario => (
            <div
              key={horario.id}
              className="bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 rounded-2xl shadow-lg p-6 flex flex-col gap-4 relative transition-transform hover:scale-[1.025] hover:shadow-2xl"
            >
              <Calendar size={40} className="text-indigo-400 mb-2 mx-auto" />
              <h2 className="text-xl font-bold text-indigo-800 text-center mb-1">
                {horario.nombre}
              </h2>
              ...
              <div className="flex justify-between items-center mt-auto">
                <span className="text-xs text-gray-500">
                  {horario.createdAt
                    ? new Date(horario.createdAt).toLocaleDateString('es-ES', {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric',
                      })
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    title="Usar"
                    className="h-9 w-9 flex items-center justify-center rounded-lg bg-indigo-100 hover:bg-indigo-200 text-indigo-700 transition"
                  >
                    <Play size={18} />
                  </button>
                  <button
                    onClick={() =>
                      (window.location.href = `/horarios/${horario.id}/editar`)
                    }
                    title="Editar"
                    className="h-9 w-9 flex items-center justify-center rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 transition"
                  >
                    <Edit3 size={18} />
                  </button>
                  <button
                    ...
                    title="Eliminar"
                    className="h-9 w-9 flex items-center justify-center rounded-lg bg-red-100 hover:bg-red-200 text-red-600 transition"
                  >
                    <Trash2 size={18} />
                  </button>
                </div>
