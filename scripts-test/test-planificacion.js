...
async function testPlanificacion() {
  try {
    ...
    const healthResponse = await fetch(
      'http://localhost:3000/api/planificaciones'
    );
    ...
    if (planificaciones.length > 0) {
      ...
      const detailResponse = await fetch(
        `http://localhost:3000/api/planificaciones/${primeraPlanificacion.id}`
      );
      const planificacionDetalle = await detailResponse.json();

      console.log(`\n4. Planificación cargada: ${planificacionDetalle.nombre}`);
      console.log(
        `   - Asignaciones: ${planificacionDetalle.asignaciones?.length || 0}`
      );

      // Verificar algunas asignaciones específicas
      if (
        planificacionDetalle.asignaciones &&
        planificacionDetalle.asignaciones.length > 0
      ) {
        console.log('\n5. Verificando asignaciones:');
        planificacionDetalle.asignaciones
          .slice(0, 5)
          .forEach((asignacion, index) => {
            console.log(
              `   ${index + 1}. OA ID: ${asignacion.oaId}, Clases: ${asignacion.cantidadClases}`
            );
          });
      }
      ...
      const oaIdsEnAsignaciones =
        planificacionDetalle.asignaciones?.map(a => a.oaId) || [];
      const oaIdsEnEjes = ejes.flatMap(eje => eje.oas.map(oa => oa.id));

      const oasNoEncontrados = oaIdsEnAsignaciones.filter(
        id => !oaIdsEnEjes.includes(id)
      );

      if (oasNoEncontrados.length === 0) {
        console.log(
          '✅ Todos los OA IDs de las asignaciones están presentes en los ejes'
        );
      } else {
      ...
      console.log(
        '🔗 URL para probar: http://localhost:3000/planificacion-anual?planificacionId=' +
          primeraPlanificacion.id
      );
    } else {
