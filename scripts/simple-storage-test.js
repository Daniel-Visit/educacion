...
async function testSimpleUpload() {
  ...
  try {
    ...
    const testContent = 'Este es un archivo de prueba para Supabase Storage - ' + new Date().toISOString();
      ...
      try {
        ...
        if (error) {
          ...
          if (error.message.includes('not found')) {
            console.log(`  ⚠️  Bucket "${bucketName}" no existe, probando siguiente...`);
            continue;
          }
        } else {
  ...
  for (const bucketName of buckets) {
    try {
      ...
      if (error) {
        ...
        console.log(`✅ Bucket "${bucketName}" existe con ${data.length} archivos`);
      }
    } catch (err) {
