...
async function insertarProfesores() {
  try {
    ...
    const profesorAsignaturasEliminadas = await prisma.profesorAsignatura.deleteMany({});
    console.log(`✅ Eliminadas ${profesorAsignaturasEliminadas.count} relaciones profesor-asignatura`);
    
    // Eliminar relaciones profesor-nivel
    const profesorNivelesEliminados = await prisma.profesorNivel.deleteMany({});
    console.log(`✅ Eliminados ${profesorNivelesEliminados.count} relaciones profesor-nivel`);
    
    // Eliminar relaciones módulo-profesor
    const moduloProfesoresEliminados = await prisma.moduloHorarioProfesor.deleteMany({});
    console.log(`✅ Eliminados ${moduloProfesoresEliminados.count} relaciones módulo-profesor`);
    ...
    console.log(`✅ Eliminados ${profesoresEliminados.count} profesores existentes`);

    // Datos de los profesores
    const hoy = new Date();
    const fecha25 = new Date(hoy.getFullYear() - 25, hoy.getMonth(), hoy.getDate());
    const fecha35 = new Date(hoy.getFullYear() - 35, hoy.getMonth(), hoy.getDate());
    const profesores = [
      {
        rut: '12345678-9',
    ...
    for (const profesor of profesores) {
      ...
      console.log(`✅ Profesor creado: ${profesorCreado.nombre} (ID: ${profesorCreado.id})`);
    }
    ...
    profesoresEnBD.forEach(prof => {
      console.log(`  - ${prof.nombre} (RUT: ${prof.rut}, Email: ${prof.email})`);
    });
...
async function mostrarProfesores() {
  try {
    ...
    if (profesores.length === 0) {
      ...
      console.log('💡 Ejecuta este script para insertar profesores de ejemplo.');
    } else {
