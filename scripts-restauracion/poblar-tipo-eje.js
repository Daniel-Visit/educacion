...
async function poblarTipoEje() {
  try {
    ...
    console.log(
      `✅ ${resultadoContenido.count} OAs actualizados con "Contenido"`
    );
    ...
    // Contar por tipo
    const conteoContenido = oasActualizados.filter(
      oa => oa.tipo_eje === 'Contenido'
    ).length;
    const conteoActitud = oasActualizados.filter(
      oa => oa.tipo_eje === 'Actitud'
    ).length;
