...
async function debugCSV() {
  try {
    ...
    const oas1 = parse(oasCSV, { columns: true, delimiter: ';', trim: true, relax_quotes: true });
    console.log('Opción 1 (relax_quotes):', oas1.length, 'filas');
    console.log('Fila 50:', oas1[49]);
    console.log('Fila 51:', oas1[50]);
    
    // Opción 2: Con relax
    const oas2 = parse(oasCSV, { columns: true, delimiter: ';', trim: true, relax: true });
    ...
    const oas3 = parse(oasCSV, { columns: true, delimiter: ';', trim: true, skip_empty_lines: true });
