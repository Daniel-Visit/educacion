...
async function configureStoragePolicies() {
  ...
  try {
    ...
    const { error: avataresError } = await supabase.storage.updateBucket(
      'avatares',
      { public: true }
    );
    ...
    const { error: archivosError } = await supabase.storage.updateBucket(
      'archivos',
      { public: false }
    );
    ...
    const { error: documentosError } = await supabase.storage.updateBucket(
      'documentos',
      { public: false }
    );
    ...
    if (documentosError) {
      console.log(
        `  ⚠️  Error actualizando bucket: ${documentosError.message}`
      );
    } else {
