...
async function consultarAmbasBases() {
  try {
    ...
    // 1. Consultar SQLite
    console.log('\n📋 SQLITE (ORIGEN):');
    const metodologiasSQLite =
      await sqlitePrisma.$queryRaw`SELECT * FROM metodologia ORDER BY id LIMIT 3`;
    console.log(JSON.stringify(metodologiasSQLite, null, 2));

    // 2. Consultar PostgreSQL
    console.log('\n📋 POSTGRESQL (DESTINO):');
    const metodologiasPostgres =
      await postgresPrisma.$queryRaw`SELECT * FROM metodologia ORDER BY id LIMIT 3`;
