...
async function verificarDatos() {
  try {
    ...
    console.log('\n📋 TABLA: asignatura');
    const asignaturas =
      await prisma.$queryRaw`SELECT * FROM asignatura ORDER BY id`;
    ...
    console.log('\n📋 TABLA: metodologia');
    const metodologias =
      await prisma.$queryRaw`SELECT * FROM metodologia ORDER BY id`;
    ...
    console.log('\n📋 TABLA: oa');
    const oas =
      await prisma.$queryRaw`SELECT id, oas_id, descripcion_oas, nivel_id, asignatura_id FROM oa ORDER BY id LIMIT 5`;
    console.log(`Total registros: ${oas.length} (mostrando primeros 5)`);
    oas.forEach(row => {
      console.log(
        `  ID: ${row.id} | OA: "${row.oas_id}" | Nivel: ${row.nivel_id} | Asignatura: ${row.asignatura_id}`
      );
    });
